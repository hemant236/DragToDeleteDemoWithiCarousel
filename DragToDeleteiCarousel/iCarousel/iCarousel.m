...
- (void)didPan:(UIPanGestureRecognizer *)panGesture
{
    if (_scrollEnabled)
    {
        switch (panGesture.state)
        {
            ...
            {
                if (_isDrag) {
                    _movingView = [self itemViewAtPoint:[panGesture locationInView:_contentView]];
                    _originalFrame = _movingView.frame;
                }
                else{
                    _dragging = YES;
                    _scrolling = NO;
                ...
                if (_isDrag) {
                    if (_canDelete) {
                        if (self.delegate && [self.delegate respondsToSelector:@selector(carousel:itemMoveWithIndex:)]) {
                            [self.delegate carousel:self itemMoveWithIndex:[self indexOfItemView:_movingView]];
                        }
                    }
                    else{
                        [_movingView setFrame:_originalFrame];
                    }
                }
            }
            ...
            default:
            {
                if (_isDrag) {
                    ...
                    _movingView.center = CGPointMake(_movingView.center.x + translatedPoint.x, _movingView.center.y + translatedPoint.y);
                    ...
                    if (self.tag == CAROUSEL_TOP) {
                        ...
                        if (_movingView.frame.origin.y > _originalFrame.origin.y + 200) {
                            _canDelete = YES;
                        }
                        else{
                            _canDelete = NO;
                    ...
                    */
                    ...
                    //NSLog(@"X %f",_movingView.center.x);
                    //NSLog(@"Y %f",_movingView.center.y);


                    //UIView *view = [self itemViewAtPoint:_movingView.center];
                    ////NSLog(@"%@",view);

                    //TODO: set delete image points here dynamically here.
                    CGRect deletedRect =   CGRectMake(100, 500, 44, 33);


                    //Check to compare the points of moving frame and and delete button
                    if (CGRectIntersectsRect(_movingView.frame,deletedRect) == TRUE){
                        _canDelete = YES;
                    ...
                    /*
                    //Check to compare the points of moving frame and and delete button
                    if (_movingView.center.x >= deletedCenter.x && _movingView.center.y > deletedCenter.y) {
                        //NSLog(@"Matched");
                        _canDelete = YES;
                    }
                    else{
                    ...
                    */
                    ////NSLog(@"dict %@",self.itemViews);
                    ////NSLog(@"Visible View %@",[self visibleItemViews]);
